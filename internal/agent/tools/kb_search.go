...
func NewKBSearchTool(
	...
	searchFn := func(ctx context.Context, args KBSearchArgs) (string, error) {
		...
		if len(results) == 0 {
			// 引导式降级：若 Agent 配置了 web_search，模型应继续联网检索；未配置则基于已有知识回答
			// 措辞为条件式，避免模型在无 web_search 工具时产生"假装调用"的幻觉
			return "知识库中没有检索到与问题相关的内容。如果你配置了网络搜索工具（web_search），且用户问题需要最新信息或实时数据，请调用 web_search 获取最新资料；否则请基于已有知识回答，并明确告知用户当前信息的局限性，不要编造。", nil
		}
