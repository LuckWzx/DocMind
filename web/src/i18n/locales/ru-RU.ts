export default {
  ...
  newUserGuide: {
    ...
    steps: {
      welcome: {
        title: 'Добро пожаловать в DocMind',
      ...
      knowledge: {
        ...
        desc: 'С этого всё начинается: загружайте документы, веб-страницы или FAQ, и DocMind автоматически разберёт и проиндексирует их. Нажмите здесь, чтобы открыть базы знаний.',
      },
      chat: {
  ...
  settings: {
    ...
    docmindCloud: {
      title: 'DocMind Cloud',
      description: 'Настройте учётные данные APPID и APPSECRET для DocMind Cloud. Данные используются для модельных сервисов и движка парсинга документов.',
      ...
      expired: 'Учётные данные DocMind Cloud истекли',
      ...
      appIdLabel: 'APPID',
      appIdDesc: 'ID приложения DocMind Cloud',
      appIdPlaceholder: 'Введите APPID',
      appSecretLabel: 'APPSECRET',
      appSecretDesc: 'Секрет приложения DocMind Cloud',
      ...
      usageSteps: '1. Сохраните APPID и APPSECRET\n2. Добавьте chat, embedding, rerank и vlm в разделе «Облачные модели» ниже\n3. Парсинг: Настройки БЗ → Движок парсинга → DocMind Cloud',
      fillRequired: 'Заполните APPID и APPSECRET',
      saveSuccess: 'Учётные данные сохранены',
      saveFailed: 'Не удалось сохранить данные',
      credentialConfigured: 'Учётные данные DocMindCloud настроены.',
      credentialExpired: 'Данные истекли. Перенастройте.',
      credentialUnconfigured: 'Учётные данные DocMindCloud не настроены. Заполните APPID и APPSECRET.',
      checkingStatus: 'Проверка статуса...',
      goToSettings: 'Перейти в настройки',
      modelHintConfigured: 'Учётные данные DocMindCloud настроены. Поддерживаемые модели см. в',
      modelHintDocsLink: 'документации API',
      modelsSection: {
        title: 'Облачные модели',
        descReady: 'Зарегистрируйте четыре стандартные модели DocMind Cloud для чата, поиска, реранкинга и зрения.',
    ...
    storage: {
      ...
      pathPrefixPlaceholder: 'напр. docmind/images',
      prefixPlaceholder: 'напр. docmind',
...
/example\.(net|org)/`,
  ...
  initialization: {
    title: 'Инициализация',
    welcome: 'Добро пожаловать в DocMind',
  ...
  auth: {
    ...
    registerSubtitle: 'Создайте аккаунт и начните работу с DocMind',
    ...
    passwordMustContainUppercase: 'Пароль должен содержать прописные буквы',
    passwordMustContainNumber: 'Пароль должен содержать цифры',
    usernameRequired: 'Введите имя пользователя',
    usernameMinLength: 'Имя пользователя должно быть не менее 4 символов',
  ...
  system: {
    ...
    versionDescription: 'Версия сервиса приложения (docmind-app)',
    frontendVersionLabel: 'Версия UI',
    frontendVersionDescription: 'Версия сборки UI (docmind-ui)',
  ...
  createChat: {
    title: 'Привет, я DocMind — ваши знания всегда под рукой',
  ...
  kbSettings: {
    ...
    parser: {
      ...
      engines: {
        ...
        docmindcloud: {
          name: 'DocMind Cloud',
          desc: 'Парсинг документов через DocMind Cloud',
        },
        markitdown: {
          name: 'MarkItDown',
  ...
  integrations: {
    ...
    api: {
      ...
      capabilityManageKbsHint: 'Позволяет управлять всем жизненным циклом баз знаний: создавать, копировать, обновлять и удалять базы знаний, а также менять их инициализацию/конфигурацию. Операции над существующими базами знаний (копирование/обновление/удаление) по-прежнему ограничены выбранной областью; создание новой базы знаний не ограничено (новая база знаний принадлежит этому пространству).',
	      ...
	      capabilityManageModelsHint: 'Управление конфигурациями моделей, учётными данными, проверками подключения и данными DocMindCloud.',
      ...
      hmacSecretDesc: 'Тот же secret, что в DocMind; только для подписи JWT на backend — не отправляйте его в заголовке запроса.',
      ...
      signedRequestStep0:
        '# 0. Подпишите JWT на backend (HS256, aud=docmind, sub=ID пользователя, tenant_id={tenantId}, exp не более 24 ч)',
    ...
    chrome: {
      ...
      subtitle:
        'Для self-hosted DocMind: вопросы в боковой панели, клиппинг страниц и Markdown-заметки в базы знаний прямо при просмотре.',
      ...
      steps: {
        ...
        port: {
          ...
          desc: 'В DocMind Desktop задайте фиксированный порт API (например 37841) в API-информации.',
        },
        install: {
    ...
    claw: {
      title: 'DocMind Skill',
      subtitle:
        'Импорт документов и гибридный поиск (вектор + ключевые слова) через REST API DocMind — загрузки, URL, Markdown и поиск.',
      ...
      steps: {
        ...
        env: {
          ...
          desc: 'Задайте DOCMIND_BASE_URL и DOCMIND_API_KEY в shell или ~/.zshrc / ~/.bashrc. Пример ниже использует текущий API URL — замените API Key на фактическое значение.',
        },
        install: {
      ...
      ecosystemNote:
        'Skill размещён на ClawHub ({\'@\'}lyingbug/docmind). Полная документация API — на странице ClawHub.',
      installCta: 'Открыть ClawHub',
      installCtaHint: 'Установка DocMind Skill · откроется в новой вкладке',
