export default {
  ...
  newUserGuide: {
    ...
    steps: {
      welcome: {
        title: "DocMind에 오신 것을 환영합니다",
        desc: "몇 단계만으로 지식 베이스, 대화, 에이전트의 핵심 사용법을 안내합니다. '다음'을 눌러 시작하세요.",
      },
      knowledge: {
        title: "지식 베이스 만들기",
        desc: "모든 것의 시작점입니다. 문서, 웹페이지, FAQ를 업로드하면 DocMind가 자동으로 분석하고 색인합니다. 여기를 클릭해 지식 베이스로 이동하세요.",
      },
      chat: {
        title: "AI 대화 시작하기",
  ...
  settings: {
    ...
    storage: {
      ...
      pathPrefixPlaceholder: '예: docmind/images',
      prefixPlaceholder: '예: docmind',
  ...
  auth: {
    ...
    registerSubtitle: "계정을 만들고 DocMind를 시작하세요",
    ...
    passwordMustContainLowercase: "비밀번호에 소문자가 포함되어야 합니다",
    passwordMustContainUppercase: "비밀번호에 대문자가 포함되어야 합니다",
    passwordMustContainNumber: "비밀번호에 숫자가 포함되어야 합니다",
    usernameRequired: "사용자명을 입력해주세요",
    usernameMinLength: "사용자명은 최소 4자여야 합니다",
  ...
  system: {
    ...
    retry: "재시도",
    versionLabel: "앱 버전",
    versionDescription: "애플리케이션 서비스(docmind-app)의 버전 번호",
    frontendVersionLabel: "UI 버전",
    frontendVersionDescription: "UI(docmind-ui) 빌드 버전 번호",
  ...
  createChat: {
    title: "안녕하세요, DocMind입니다 — 당신의 지식을 손끝에",
  ...
  integrations: {
    ...
    api: {
      ...
      capabilityManageKbsHint: "지식베이스 전체 수명 주기를 관리할 수 있습니다: 생성, 복사, 수정, 삭제 및 초기화/설정 변경. 기존 지식베이스에 대한 작업(복사/수정/삭제)은 선택한 범위로 제한되며, 새 지식베이스 생성은 제한되지 않습니다(새 지식베이스는 이 스페이스에 속함).",
	      ...
	      capabilityManageModelsHint: "모델 설정, 자격 증명, 연결 테스트 및 DocMindCloud 자격 증명을 관리합니다.",
      ...
      signedFlowDetail:
        "HMAC secret은 DocMind와 신뢰할 수 있는 백엔드에만 보관하세요. 요청에 넣거나 브라우저에 배포하지 마세요. 요청 헤더에는 secret이 아니라 해당 secret으로 서명한 JWT 문자열을 넣습니다(발급마다 또는 만료 후 변경). JWT에는 sub(외부 사용자 ID), tenant_id, aud=docmind, exp(최대 24시간)가 필요합니다.",
      ...
      hmacSecretDesc: "DocMind에 저장된 것과 동일한 secret. 백엔드에서 JWT 서명에만 사용하며 요청 헤더로 보내지 않습니다.",
      ...
      signedRequestStep0:
        "# 0. 백엔드에서 JWT 서명 (HS256, aud=docmind, sub=사용자 ID, tenant_id={tenantId}, exp 최대 24시간)",
    ...
    chrome: {
      title: "지식 관리 어시스턴트",
      subtitle:
        "자체 호스팅 DocMind와 함께: 사이드바에서 질문하고, 웹 페이지를 클리핑하며, Markdown 메모를 지식베이스에 저장하세요.",
      ...
      steps: {
        ...
        port: {
          title: "데스크톱: 고정 포트(권장)",
          desc: "DocMind 데스크톱에서는 API 정보에서 고정 포트(예: 37841)를 설정하세요.",
        },
        install: {
          title: "Chrome 확장 설치",
    ...
    claw: {
      title: "DocMind Skill",
      subtitle:
        "DocMind REST API로 문서를 가져오고 하이브리드 검색(벡터+키워드)을 수행합니다. 파일/URL/Markdown 업로드 및 검색에 사용.",
      ...
      steps: {
        ...
        env: {
          title: "환경 변수 설정",
          desc: "셸 또는 ~/.zshrc, ~/.bashrc에 DOCMIND_BASE_URL과 DOCMIND_API_KEY를 설정하세요. 아래 예시는 현재 API 주소를 사용하며, API Key는 실제 값으로 바꾸세요.",
        },
        install: {
          title: "Skill 설치",
      ...
      ecosystemNote: "Skill은 ClawHub({'@'}lyingbug/docmind)에 호스팅됩니다. 전체 API 문서는 ClawHub 페이지를 참고하세요.",
      installCta: "ClawHub 열기",
      installCtaHint: "DocMind Skill 설치 · 새 탭에서 열림",
