...
export const DOCMIND_CLOUD_BASE_URL = 'https://docmind.weixin.qq.com'
export const DOCMIND_CLOUD_PROVIDER = 'docmindcloud'
...
export function isDocMindCloudModel(model: ModelConfig): boolean {
  return model.parameters?.provider === DOCMIND_CLOUD_PROVIDER
}

/** Returns kinds that already have a DocMindCloud model of the matching backend type. */
export function existingWkcKinds(models: ModelConfig[]): Set<WkcModelKind> {
  const found = new Set<WkcModelKind>()
  for (const model of models) {
    if (!isDocMindCloudModel(model)) continue
...
export function buildWkcModelConfig(
  ...
  const parameters: ModelConfig['parameters'] = {
    base_url: DOCMIND_CLOUD_BASE_URL,
