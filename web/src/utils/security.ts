...
const protectedFileCacheState = (() => {
  ...
  const scope = window as typeof window & {
    __docmindProtectedFileCacheV1__?: ProtectedFileCacheState;
  };
  scope.__docmindProtectedFileCacheV1__ ||= fresh();
  return scope.__docmindProtectedFileCacheV1__;
})();
...
function getProtectedFileRequestHeaders(): Record<string, string> {
  ...
  try {
    const token = (localStorage.getItem('docmind_token') || '').trim();
    if (token) {
      headers['Authorization'] = `Bearer ${token}`;
    }

    const selectedTenantId = (localStorage.getItem('docmind_selected_tenant_id') || '').trim();
    if (selectedTenantId) {
      ...
      // active tenant into docmind_tenant, leaving authenticated file
