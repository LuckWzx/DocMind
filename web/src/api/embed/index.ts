...
export const EMBED_CHAT_SESSION_STORAGE_PREFIX = 'docmind-embed-session:'

/** localStorage key prefix for anonymous embed visitor ids (per channel). */
export const EMBED_VISITOR_STORAGE_PREFIX = 'docmind-embed-visitor:'
...
const EMBED_MSG_SOURCE = 'docmind-embed'
const EMBED_HOST_SOURCE = 'docmind-host'
...
export function buildWidgetSnippet(
  ...
  const attrs = [
    `src="${escapeHtmlAttr(`${base}/docmind-widget.js`)}"`,
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER = 'https://your-backend.example.com/docmind/embed-token'
...
export function buildSecureServerNodeExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  return [
    `// Node/Express — keep DOCMIND_PUBLISH_TOKEN only on the server (env var).`,
    `app.get('/docmind/embed-token', async (req, res) => {`,
    ...
    `    method: 'POST',`,
    `    headers: {`,
    `      Authorization: 'Embed ' + process.env.DOCMIND_PUBLISH_TOKEN,`,
...
export function buildSecureServerGoExample(channelId: string, opts?: { baseUrl?: string }) {
  ...
  const exchangeUrl = `${base}/api/v1/embed/${channelId}/exchange`
  return [
    `// Go net/http — keep DOCMIND_PUBLISH_TOKEN only on the server (env var).`,
    ...
    `  req.Header.Set("Authorization", "Embed "+os.Getenv("DOCMIND_PUBLISH_TOKEN"))`,
