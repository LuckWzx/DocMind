...
export interface InitializeDocMindCloudRequest {
  app_id: string
  app_secret: string
}

// 仅保存 DocMindCloud 凭证，不自动创建模型
export function saveDocMindCloudCredentials(data: InitializeDocMindCloudRequest): Promise<{ success: boolean; message: string }> {
  return new Promise((resolve, reject) => {
    post('/api/v1/docmindcloud/credentials', data)
      .then((response: any) => {
        if (response.success) {
          resolve(response)
        } else {
      ...
      .catch((error: any) => {
        console.error('Failed to save DocMindCloud credentials:', error)
        reject(error)
      })
  })
}

export interface DocMindCloudStatusResult {
  has_models: boolean
  needs_reinit: boolean
  reason?: string
}

export function getDocMindCloudStatus(): Promise<DocMindCloudStatusResult> {
  return new Promise((resolve, reject) => {
    get('/api/v1/models/docmindcloud/status')
      .then((response: any) => {
