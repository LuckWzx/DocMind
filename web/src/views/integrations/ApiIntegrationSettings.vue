...
type DocMindDesktopWindow = Window & {
  __DOCMIND_API_BASE__?: string
  __DOCMIND_API_LAN_BASE__?: string
  go?: {
    main?: {
      App?: {
...
// Send on each DocMind API request:
...
async function tryLoadWailsApiBaseURL() {
  const win = window as DocMindDesktopWindow
  for (let i = 0; i < 40; i++) {
    const injected = win.__DOCMIND_API_BASE__
...
async function tryLoadWailsLanHints(win: DocMindDesktopWindow) {
  const injectedLan = win.__DOCMIND_API_LAN_BASE__
...
function desktopPortBindingsAvailable(win: DocMindDesktopWindow) {
...
function desktopBindPublicBindingsAvailable(win: DocMindDesktopWindow) {
  const app = win.go?.main?.App
  return (
...
const onDesktopBindPublicChange = async (value: boolean) => {
  ...
  const fn = (window as DocMindDesktopWindow).go?.main?.App?.SetDesktopHTTPBindPublicSetting
...
const saveDesktopPort = async () => {
  ...
  const fn = (window as DocMindDesktopWindow).go?.main?.App?.SetDesktopHTTPPortSetting
  if (typeof fn !== 'function') return
  try {
...
function openApiDoc() {
  window.open('https://github.com/Tencent/DocMind/blob/main/docs/api/README.md', '_blank')
}
