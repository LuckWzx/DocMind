<template>
  <div class="docmindcloud-settings">
    <div class="section-header">
      <h2>{{ $t('settings.docmindCloud.title') }}</h2>
      <p class="section-description">
        {{ $t('settings.docmindCloud.description') }}
        <a
          class="doc-link"
        ...
        >
          {{ $t('settings.docmindCloud.viewDocs') }}
    ...
    <div v-if="credentialState === 'unconfigured'" class="credential-status unconfigured">
      ...
      <span>{{ $t('settings.docmindCloud.unconfigured') }}</span>
    </div>
    ...
    <div v-else-if="credentialState === 'expired'" class="credential-warning">
      ...
      <div class="warning-text">
        <strong>{{ $t('settings.docmindCloud.expired') }}</strong><br />
        {{ reinitReason || $t('settings.docmindCloud.expiredDefault') }}
      </div>
    </div>
    ...
    <div v-else-if="credentialState === 'configured'" class="credential-status success">
      ...
      <span class="status-text">{{ $t('settings.docmindCloud.configured') }}</span>
      <t-button
        v-if="!formExpanded"
        variant="outline"
        theme="default"
      ...
      >
        ...
        {{ $t('settings.docmindCloud.reconfigure') }}
      </t-button>
    </div>
    ...
    <div v-if="formExpanded" class="settings-group">
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.docmindCloud.appIdLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.docmindCloud.appIdDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appId"
            :placeholder="$t('settings.docmindCloud.appIdPlaceholder')"
            autocomplete="off"
            style="width: 280px;"
          />
        </div>
      ...
      <div class="setting-row">
        <div class="setting-info">
          <label class="setting-label">{{ $t('settings.docmindCloud.appSecretLabel') }}</label>
          <p class="setting-desc">{{ $t('settings.docmindCloud.appSecretDesc') }}</p>
        </div>
        <div class="setting-control">
          <t-input
            v-model="form.appSecret"
            type="password"
            :placeholder="$t('settings.docmindCloud.appSecretPlaceholder')"
      ...
      <div class="setting-row action-row">
        <div class="setting-info">
          <p class="setting-desc">{{ $t('settings.docmindCloud.saveHint') }}</p>
        </div>
        <div class="setting-control">
          <t-button
            theme="primary"
            ...
            @click="handleSave"
          >
            {{ $t('settings.docmindCloud.saveBtn') }}
          </t-button>
        </div>
      </div>
    </div>
    ...
    >
      <div class="models-section__header">
        <h3 class="models-section__title">{{ $t('settings.docmindCloud.modelsSection.title') }}</h3>
        <p class="models-section__desc">
          {{
            credentialState === 'configured'
              ? $t('settings.docmindCloud.modelsSection.descReady')
              : $t('settings.docmindCloud.modelsSection.descPending')
          }}
        </p>
      </div>
        ...
        >
          ...
          <div class="model-row__action">
            <t-tag
              ...
              size="small"
            >
              {{ $t('settings.docmindCloud.modelsSection.statusAdded') }}
            </t-tag>
            <t-popconfirm
              v-else-if="credentialState === 'configured'"
              :content="$t('settings.docmindCloud.modelsSection.confirmAddOne', {
                type: kindLabel(kind),
                name: WKC_MODEL_NAME_BY_KIND[kind],
              })"
              :confirm-btn="{ content: $t('settings.docmindCloud.modelsSection.addOne'), theme: 'primary' }"
            ...
            >
              <t-button
                ...
                @click.stop
              >
                {{ $t('settings.docmindCloud.modelsSection.addOne') }}
              </t-button>
            </t-popconfirm>
            <span v-else class="model-row__pending">
              {{ $t('settings.docmindCloud.modelsSection.statusPending') }}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        <t-popconfirm
          :content="$t('settings.docmindCloud.modelsSection.confirmAddAll', { count: missingKinds.length })"
          :confirm-btn="{
            content: $t('settings.docmindCloud.modelsSection.addAllConfirm'),
            theme: 'primary',
          }"
          :cancel-btn="{ content: $t('common.cancel') }"
          placement="top"
        ...
        >
          ...
          >
            {{ $t('settings.docmindCloud.modelsSection.addAllBtn', { count: missingKinds.length }) }}
          </t-button>
        </t-popconfirm>
      </div>
      ...
      >
        ...
        {{ $t('settings.docmindCloud.modelsSection.allReady') }}
      </p>
    </section>

    <!-- 使用说明 -->
    <div class="usage-hint">
      <p class="hint-title">{{ $t('settings.docmindCloud.usageTitle') }}</p>
      <p class="hint-text" v-html="$t('settings.docmindCloud.usageSteps').replace(/\n/g, '<br />')" />
    </div>
  </div>
</template>
...
import { saveDocMindCloudCredentials, getDocMindCloudStatus, createModel, listModels } from '@/api/model'
import { testEmbeddingModel } from '@/api/initialization'
import {
  WKC_MODEL_KINDS,
  WKC_MODEL_NAME_BY_KIND,
  DOCMIND_CLOUD_BASE_URL,
  DOCMIND_CLOUD_PROVIDER,
  buildWkcModelConfig,
  existingWkcKinds,
  type WkcModelKind,
} from '@/utils/docmindCloudModels'
...
const kindDisplayName = (kind: WkcModelKind) =>
  t(`settings.docmindCloud.addModelsDisplayName.${kind}`)
...
const resolveEmbeddingDimension = async (): Promise<number> => {
  const result = await testEmbeddingModel({
    ...
    baseUrl: DOCMIND_CLOUD_BASE_URL,
    provider: DOCMIND_CLOUD_PROVIDER,
  })
  if (!result.available || !result.dimension) {
    throw new Error(result.message || t('settings.docmindCloud.addModelsEmbeddingFailed'))
  }
  ...
  try {
    for (const kind of targets) {
      try {
        ...
        console.error(`Failed to create DocMindCloud ${kind} model:`, err)
        failed += 1
      }
    }

    if (success > 0 && failed === 0) {
      MessagePlugin.success(t('settings.docmindCloud.addModelsSuccess', { count: success }))
    } else if (success > 0) {
      MessagePlugin.warning(t('settings.docmindCloud.addModelsPartial', { success, failed }))
    } else {
      MessagePlugin.error(t('settings.docmindCloud.addModelsFailed'))
    }
  } finally {
...
const handleSave = async () => {
  if (!form.value.appId || !form.value.appSecret) {
    MessagePlugin.warning(t('settings.docmindCloud.fillRequired'))
    return
  }
  saving.value = true
  try {
    await saveDocMindCloudCredentials({
      app_id: form.value.appId,
      app_secret: form.value.appSecret,
    })
    MessagePlugin.success(t('settings.docmindCloud.saveSuccess'))
    form.value.appId = ''
    ...
    MessagePlugin.error(err?.message || t('settings.docmindCloud.saveFailed'))
  } finally {
    saving.value = false
  }
}
