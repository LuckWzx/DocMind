 ...
 *
 * Storage layout: DocMind_${userId}_${suffix}, where userId is the active
 ...
 *   - Legacy un-namespaced keys (DocMind_${suffix}) from earlier branch
...
export function readUserId(): string {
  try {
    const raw = localStorage.getItem('docmind_user')
    if (!raw) return 'anon'
...
export function safeSetItem(key: string, value: string): void {
  try {
    ...
    console.warn(`[DocMind] failed to persist preference "${key}":`, err)
  }
}
...
export function userKey(suffix: string): string {
  return `DocMind_${readUserId()}_${suffix}`
}
...
export function migratePreferencesIntoUser(): void {
  ...
  for (const suffix of PREFERENCE_SUFFIXES) {
    const target = `DocMind_${userId}_${suffix}`
    const targetExists = safeGetItem(target) !== null

    const anonKey = `DocMind_anon_${suffix}`
    const legacyKey = `DocMind_${suffix}`
