...
export const useAuthStore = defineStore('auth', () => {
  ...
  //
  // SECURITY: This value is read from localStorage (`docmind_memberships`)
  ...
  const setTenant = (tenantData: TenantInfo | null) => {
    ...
    if (tenantData) {
      localStorage.setItem('docmind_tenant', JSON.stringify(tenantData))
    } else {
      localStorage.removeItem('docmind_tenant')
  ...
  const setToken = (tokenValue: string) => {
    ...
    localStorage.setItem('docmind_token', tokenValue)
  }

  const setRefreshToken = (refreshTokenValue: string) => {
    refreshToken.value = refreshTokenValue
    localStorage.setItem('docmind_refresh_token', refreshTokenValue)
  }

  const setKnowledgeBases = (kbList: KnowledgeBaseInfo[]) => {
    // 确保输入是数组
    knowledgeBases.value = Array.isArray(kbList) ? kbList : []
    localStorage.setItem('docmind_knowledge_bases', JSON.stringify(knowledgeBases.value))
  }

  const setCurrentKnowledgeBase = (kb: KnowledgeBaseInfo | null) => {
    currentKnowledgeBase.value = kb
    if (kb) {
      localStorage.setItem('docmind_current_kb', JSON.stringify(kb))
    } else {
      localStorage.removeItem('docmind_current_kb')
    }
  }
  ...
  const clearTenantScopedClientState = () => {
    try {
      localStorage.removeItem('docmind_last_chat_model_id')
      localStorage.removeItem('docmind_current_kb')
      const raw = localStorage.getItem('DocMind_settings')
      if (raw) {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          localStorage.setItem('DocMind_settings', JSON.stringify(parsed))
        }
      }
    } catch (e) {
  ...
  const setSelectedTenant = (tenantId: number | null, tenantName: string | null = null) => {
    ...
    if (tenantId !== null) {
      localStorage.setItem('docmind_selected_tenant_id', String(tenantId))
      if (tenantName) {
        localStorage.setItem('docmind_selected_tenant_name', tenantName)
      }
    } else {
      localStorage.removeItem('docmind_selected_tenant_id')
      localStorage.removeItem('docmind_selected_tenant_name')
    }
    if (tenantChanged) {
  ...
  const setMemberships = (
    ...
    localStorage.setItem('docmind_memberships', JSON.stringify(memberships.value))
  }
  ...
  const setLiteMode = (value: boolean) => {
    ...
    if (value) {
      localStorage.setItem('docmind_lite_mode', 'true')
    } else {
      localStorage.removeItem('docmind_lite_mode')
    }
  }
  ...
  const logout = () => {
    ...
    localStorage.removeItem('docmind_knowledge_bases')
    localStorage.removeItem('docmind_current_kb')
    localStorage.removeItem('docmind_selected_tenant_id')
    localStorage.removeItem('docmind_selected_tenant_name')
    localStorage.removeItem('docmind_memberships')
    localStorage.removeItem('docmind_lite_mode')
    isLiteMode.value = false
    try {
      sessionStorage.removeItem('docmind_lite_last_path')
    } catch {
      /* ignore */
    }
    reloadUserPreferences()
  ...
  const initFromStorage = () => {
    ...
    const storedTenant = localStorage.getItem('docmind_tenant')
    const storedToken = localStorage.getItem('docmind_token')
    const storedRefreshToken = localStorage.getItem('docmind_refresh_token')
    const storedKnowledgeBases = localStorage.getItem('docmind_knowledge_bases')
    const storedCurrentKb = localStorage.getItem('docmind_current_kb')
    const storedSelectedTenantId = localStorage.getItem('docmind_selected_tenant_id')
    const storedSelectedTenantName = localStorage.getItem('docmind_selected_tenant_name')
    ...
    const storedMemberships = localStorage.getItem('docmind_memberships')
    ...
    isLiteMode.value = localStorage.getItem('docmind_lite_mode') === 'true'
  }
