...
│   │       ├── tag/                  # 标签模块（独立 CRUD）
│   │       ├── vectorstore/          # 向量存储配置
│   │       └── websearch/            # 网页搜索（Web Search Provider 管理）
│   ├── app/                          # 应用生命周期管理
│   │   ├── app.go                    # 初始化、依赖注入、AutoMigrate、自动启动DocReader
│   │   └── mcp_seed.go               # MCP 默认服务种子数据
│   ├── llm/                           # LLM 模型工厂层
...
│   │   └── tools/                     # Agent 工具集
│   │       ├── analysis_validate.go   # 数据分析结果校验
│   │       ├── data_analysis.go       # 数据分析工具（DuckDB 会话）
...
│   ├── model/                        # 数据模型
│   │   ├── dto/                      # 数据传输对象
│   │   │   ├── request/              # 请求 DTO（11个模块：auth / chunker / faq / knowledge / knowledge_base / mcp / model / tag / user / vector_store / web_search_provider）
│   │   │   └── response/             # 响应 DTO（10个模块：chunker / faq / knowledge / knowledge_base / mcp / model / tag / user / vector_store / web_search_provider）
│   │   └── entity/                   # 数据库实体（GORM）
...
│   │       ├── mcp_tool_approval.go  # MCP 工具审批记录
...
│   ├── MCP集成指南-openmaic-classroom.md # MCP 集成指南（openmaic-classroom 技能）
